...
//! （字段名/大小写、content 结构、isError 语义）。这里直接 spawn `texpresso-mcp`、按 stdio
...
impl Fixture {
    fn new(name: &str) -> Self {
        let dir = std::env::temp_dir().join(format!("texpresso-mcp-{name}-{}", std::process::id()));
...
impl Client {
    fn start(project: &Path, config: &Path) -> Self {
        let exe = env!("CARGO_BIN_EXE_texpresso-mcp");
...
fn handshake(c: &mut Client) -> Value {
    let init = c.send(json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": { "name": "texpresso-test", "version": "0" }
        }
    }));
    // 通知：无响应
...
fn handshake_reports_server_info_and_tool_capability() {
    ...
    assert_eq!(result["serverInfo"]["name"], "texpresso");
...
fn project_can_be_switched_per_call() {
    ...
    let other = std::env::temp_dir().join(format!("texpresso-mcp-other-{}", std::process::id()));
...
/// 真实编译：需要 latexmk/xelatex（`cargo test -p texpresso-server -- --ignored`）。
#[test]
#[ignore]
