    ...
    let response = match method {
        "initialize" => {
            ...
            ok(id, json!({
                ...
                "serverInfo": { "name": "texpresso", "version": env!("CARGO_PKG_VERSION") },
                "instructions": "TeXPresso headless：驱动 LaTeX 项目「读 → 改 → 编译验证 → 修」。先 project_open（或直接给 compile 传 project），再 compile（默认完整编译、返回结构化错误与诊断），改完再 compile。"
            }))
        }
        "ping" => ok(id, json!({})),
        "tools/list" => {
