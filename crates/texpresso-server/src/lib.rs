//! TexPresso 无 GUI 交互层（roadmap ⑥）：**headless 服务 + CLI + MCP**。
//!
...
//!    都在 `texpresso_core::synctex`，GUI 命令面与这里调同一份，避免行为漂移（原计划 P0-3）。
...
use texpresso_core::settings::{ProjectOverrides, Settings};
use texpresso_core::synctex::{pdf_path_for_root, resolve_inverse, synctex_data_path, SourcePosition,
    SyncTexPosition, SyncTexProvider};
use texpresso_core::types::{
    CompileKind, CompileOutcome, CompileRequest, ErrorEntry, OutlineNode,
};
use texpresso_infra::fs::TokioFs;
use texpresso_infra::runner::LatexmkRunner;
use texpresso_infra::storage::SettingsStorage;
use texpresso_infra::synctex::SyncTexCli;
...
impl Session {
    /// 构造会话：`config_dir` 传**与 GUI 相同的**应用配置目录（Windows 下 `%APPDATA%\com.texpresso.app`）。
        ...
        let report = match outcome {
            ...
            CompileOutcome::IoError { message } => CompileReport {
                ...
                errors: vec![ErrorEntry {
                    ...
                    file: None,
                    line: None,
                    kind: texpresso_core::types::ErrorKind::Io,
                    diagnosis: None,
                }],
                pdf_path: None,
                elapsed_ms,
    ...
    pub async fn list_tex_files(&self) -> Result<Vec<PathBuf>, ServerError> {
        let project = self.project_or_err()?;
        texpresso_core::project::collect_tex_files(self.fs.as_ref(), &project.root)
            .await
            .map_err(|e| ServerError::Internal(format!("扫描项目失败：{e}")))
    }
    ...
    pub async fn write(&self, path: &Path, content: &str) -> Result<PathBuf, ServerError> {
        ...
        let target = resolve_creatable_in_project(self.fs.as_ref(), &project.root, path)
            .await
            .map_err(|e| match e {
                texpresso_core::project::PathError::NotFound => ServerError::NotFound(format!(
    ...
    pub async fn synctex_inverse(
        ...
        x: f32,
        y: f32,
    ) -> Result<texpresso_core::synctex::InverseResolution, ServerError> {
        let project = self.project_or_err()?;
        let pdf = pdf_path_for_root(&project.root, project.root_file.as_deref());
        // 同步数据缺失时给更准确的话（"先编译一次"），而不是笼统的 CLI 报错
        if !self.fs.exists(&pdf).await.unwrap_or(false) {
            return Ok(texpresso_core::synctex::InverseResolution {
                source: None,
    ...
    fn sync_err(&self, e: texpresso_core::synctex::SyncTexError, pdf: &Path, project: &ProjectState) -> ServerError {
        let _ = (pdf, project);
...
fn engine_name(s: &Settings) -> String {
    match s.compile.engine {
        texpresso_core::types::Engine::XeLaTeX => "xelatex",
        texpresso_core::types::Engine::PdfLaTeX => "pdflatex",
        texpresso_core::types::Engine::LuaLaTeX => "lualatex",
    }
    .to_string()
}
...
fn path_err(e: texpresso_core::project::PathError, path: &Path, what: &str) -> ServerError {
    use texpresso_core::project::PathError;
    match e {
...
///
/// 环境变量 `TEXPRESSO_CONFIG_DIR` 可覆盖（测试/CI 用；GUI 侧无此变量，因此不影响一致性）。
pub fn default_config_dir() -> PathBuf {
    if let Ok(dir) = std::env::var("TEXPRESSO_CONFIG_DIR") {
        if !dir.trim().is_empty() {
            return PathBuf::from(dir);
        }
    }
    const ID: &str = "com.texpresso.app";
