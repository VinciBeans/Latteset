//! `texpresso-cli`：headless 命令行入口（roadmap ⑥）。
//!
//! 约定（面向 Agent/脚本）：
...
use texpresso_server::{ensure_open, ServerError, Session};

const USAGE: &str = "\
texpresso-cli —— TexPresso headless CLI（读 → 改 → 编译验证 → 修）

用法：
  texpresso-cli [--project <目录>] [--config-dir <目录>] <命令> [参数]
...
全局参数：
  ...
  --config-dir <目录>  应用配置目录（默认与 GUI 相同；也可用环境变量 TEXPRESSO_CONFIG_DIR）
";
