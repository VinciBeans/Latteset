//! `texpresso-mcp`：MCP server（stdio 传输），把 headless 服务暴露成 Agent 可直接调用的工具。
//!
//! 用法（harness 侧配置成 stdio MCP server 即可）：
//! ```text
//! texpresso-mcp [--project <dir>] [--config-dir <dir>]
//! ```
//! - 一行一条 JSON-RPC 消息（MCP stdio 约定）；**stdout 只有协议消息**，日志一律 stderr。
//! - `--project` 只影响"未显式传 project 参数"的工具调用（预打开该项目，工具调用可省略 `project`）。

use std::path::PathBuf;
use texpresso_server::mcp::handle_message;
use texpresso_server::{absolutize, Session};
use tokio::io::{AsyncBufReadExt, AsyncWriteExt, BufReader};

const USAGE: &str = "\
texpresso-mcp —— TexPresso MCP server（stdio）

用法：texpresso-mcp [--project <目录>] [--config-dir <目录>]

  --project <目录>     预打开项目（省略时由工具参数 project / 当前目录决定）
  --config-dir <目录>  应用配置目录（默认与 GUI 相同；也可用 TEXPRESSO_CONFIG_DIR）
  --help               本帮助
...
async fn main() {
    ...
    if let Some(project) = &project {
        if let Err(e) = session.open_project(&absolutize(project)).await {
            ...
            eprintln!("[texpresso-mcp] 预打开项目失败：{e}");
        }
    }
