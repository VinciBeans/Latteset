//! headless 服务的单元/集成测试（`cargo test -p texpresso-server`）。
//!
//! 不需要 LaTeX 的部分（打开/读/写/树/大纲/错误形状）在默认测试里跑；
//! 真编译与 SyncTeX 相关用例标 `#[ignore]`（与 `texpresso-infra` 的约定一致，需要本机 TeX Live）：
//! `cargo test -p texpresso-server -- --ignored`。

use super::*;
use std::fs;

/// 临时项目目录（中文名 + .texpresso 覆盖，覆盖路径与设置读写的真实形态）。
struct TempProject {
    dir: PathBuf,
}
...
impl TempProject {
    fn new(name: &str) -> Self {
        let dir = std::env::temp_dir().join(format!("texpresso-server-{name}-{}", std::process::id()));
...
async fn open_project_detects_unique_root_and_reports_settings() {
    ...
    assert_eq!(report.settings.compile.engine, texpresso_core::types::Engine::XeLaTeX);
}
...
async fn project_override_root_file_wins() {
    ...
    p.put("a.tex", "\\documentclass{article}\n\\begin{document}x\\end{document}\n");
    p.put("b.tex", "\\documentclass{article}\n\\begin{document}y\\end{document}\n");
    p.put(".texpresso/settings.json", "{ \"root_file\": \"b.tex\" }");
...
/// 一个测试同时覆盖两种情形：**必须合并**——`TEXPRESSO_CONFIG_DIR` 是进程级环境变量，
/// 拆成两个测试会在并行执行时互相污染（实测踩过）。
#[test]
fn default_config_dir_matches_gui_and_honours_env_override() {
    // 1) 无覆盖：末段必须是应用 identifier（Windows = %APPDATA%\com.texpresso.app，即 GUI 的 app_config_dir）
    unsafe { std::env::remove_var("TEXPRESSO_CONFIG_DIR") };
    let dir = default_config_dir();
    assert_eq!(
        dir.file_name().map(|n| n.to_string_lossy().into_owned()).as_deref(),
        Some("com.texpresso.app"),
        "{dir:?}"
    );

    // 2) 有覆盖（测试/CI 隔离用）：直接返回它；空串视为未设置
    unsafe { std::env::set_var("TEXPRESSO_CONFIG_DIR", "/tmp/texpresso-cfg-probe") };
    assert_eq!(default_config_dir(), PathBuf::from("/tmp/texpresso-cfg-probe"));
    unsafe { std::env::set_var("TEXPRESSO_CONFIG_DIR", "   ") };
    assert_eq!(
