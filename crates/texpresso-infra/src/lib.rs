//! texpresso-infra：**基础设施层**（ADR-0010）。
//!
//! 位置：位于 texpresso-core（纯领域）与 src-tauri（接线薄壳）之间，与 core 同级；
//! 是**外部依赖与文件系统的唯一落点**。
//!
//! 纪律：
//! - 上层（src-tauri 的 commands 编排）不得直接调用 OS / 外部进程 API——
//!   文件读写走 [texpresso_core::project::FileSystem]（本 crate 提供 TokioFs 实现），
//!   进程走 [texpresso_core::scheduler::CompileRunner] /
//!   [texpresso_core::synctex::SyncTexProvider]（本 crate 实现）；
