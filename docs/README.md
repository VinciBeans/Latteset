...
| [tectonic-library-plan.md](./tectonic-library-plan.md) | **实施中**：Tectonic 库形态（路径 B）集成方案——§3 落点与调用序列、§5 bundle/缓存口径、§6 分阶段闸门与复核命令（**§6.1 = 已落地状态**：能真编出 PDF、4 个实测缺陷与修法、release 性能对照）、§12 风险 |
| [adr/](./adr/) | 决策记录（ADR），当前 12 项 |
...
新成员：`根 README → CONTEXT.md → design.md → architecture.md → architecture-diagram.md → modules.md → adr/0001-0012`
