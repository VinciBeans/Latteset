...
1. **先做 B（C1）**：后端在"新 XDV 的页哈希与上一次全同"时**不发** `pdf-updated`（顺带：状态栏可以提示"内容无可见变化"）。零风险、可单测（构造两份页哈希相同的 XDV），省掉每次无意义编辑的预览重载（103ms + 视觉跳动）。
2. **再做 ⑪ 的 C**：把"变化页集合"接进 `PreviewPane`（与 ⑪ 的滚动/缩放保持一起做，别单独动渲染契约）。收益上限由 §2.3-2 的编辑位置分布决定——**建议先在真机上量一次"真实编辑的命中率分布"**（用 G2 的 `--diff` 挂到编译后钩子上，只记录不改行为），再决定投入。
3. **A 视情况**：若要省那 0.65–0.94s 的转换，需要把 Quick 从"引擎内部转 PDF"改成"`-no-pdf` + 条件 `xdvipdfmx`"——它同时会让 ⑤（追加式解析）第一次拥有真实消费方（编译期读 XDV）。**这是本轮研究里唯一能让阶段 3 的积木派上用场的路径**，但收益是每次编辑 ~0.7s，优先级排在 B/C 之后。
