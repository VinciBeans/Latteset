...
| 3 | **半成品可读**（G2 的核心） | ✅ 截断到 10/33/50/75/90/99.9% → 分别解出 17/61/92/139/166/185 页，且**每页哈希与完整文件逐页相同**（17/17 … 185/185）；末页不完整即丢弃 = 上游 `incdvi` 的回滚规则。**2026-09 加强**（[阶段 3 报告](./stage3-incremental-output-parsing.md) §6）：采样点从 6 个扩到 **1122 个 4KB 网格点 / 71993 页比对 / 0 处不一致**——期间**发现并修复了两个静默错误**：截断点落在一条指令中间、或恰好扫到缓冲区末尾时，解析器会把**内容被截断的半页**当成完整页推进页索引（hash 也错）。上面那 6 个采样点只是恰好没踩到 |
| 4 | 页包自包含 | ✅ 单页字节切片（只加文件前导）独立解析成 1 页且哈希相同（第 1/94/186 页） |
| 5 | 成本 | ✅ 全量 186 页/4.41MB = **2.7ms（1.6 GB/s）**；按 0.5MB 落盘粒度增量 ≈**0.3ms/次**（对比 dvisvgm 每次预扫 0.7–1.0s）。**2026-09**：0.3ms/次当时是**吞吐推算**，现已由真正的追加式解析器实测取代——141 轮 flush 下**增量累计恒定 6.6–8.1ms**（与轮数无关）、每轮全量累计线性涨到 260ms（[阶段 3 报告](./stage3-incremental-output-parsing.md) §5） |
...
| 9 | 与 SyncTeX 比 | 🟡 互补而非替代：synctex.gz 179KB（解压 1.14MB、gunzip 3.4ms、61 个 `Input:` 源文件记录），给的是**源码↔坐标**；XDV 页哈希给的是**页字节是否变**。**页级对拍 2026-09 已做**：两个新样本完全自洽（等长替换 → 差分 1 页 = 19、SyncTeX = 19；文首插入 → 差分 120/125 页、首个命中 = 5、SyncTeX = 5），见 [阶段 3 报告](./stage3-incremental-output-parsing.md) §7 |
...
→ 两者**互补**：要"页是否变"用 XDV 页哈希（便宜、直接），要"从源码找到页/从页找到源码"用 SyncTeX。**本次未做两者对拍**（见 §9）→ ✅ **2026-09 已补**（[阶段 3 报告](./stage3-incremental-output-parsing.md) §7：两个样本完全自洽）。
...
3. ~~**未做 SyncTeX 页级对拍**~~ ✅ **已补（2026-09）**：[阶段 3 报告](./stage3-incremental-output-parsing.md) §7 两个样本完全自洽（等长替换 19=19；文首插入首个命中 5=5）。**仍未做**：插入导致**页数变化**（其后页号整体平移）时的对拍。
...
6. ~~**"增量追加"尚未实现**~~ ✅ **已实现（2026-09，上游阶段 3 的技术要件）**：`scripts/xdv-inc.mjs` 的 `createIncXdvIndex`（offset 前进 + BOP/EOP 记录 + 缓冲区变短的回滚），4 种喂入序列与全量解析逐字段对拍一致；141 轮 flush 下增量累计恒定 6.6–8.1ms、每轮全量线性涨到 260ms。**但结论是"入库不接线"**——没有消费方（编译期页进度已由 `[N]` 标记满足）。见 [阶段 3 报告](./stage3-incremental-output-parsing.md)。实现时还暴露并修复了本文 §3.2 那两个静默错误（见判据 3 的 2026-09 补注）。
