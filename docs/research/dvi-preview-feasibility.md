...
2. **触发与预算**：每 N 页 / 每 T ms，且限制总次数；转换要放工作线程、可取消，别和排版抢 CPU（子进程档每次 ~0.55 s，阈值要更保守）。→ **2026-09-16 已定**：不是"抢 CPU"而是**进程内全局锁**（§11），所以"放工作线程"这条走不通；落在**趟边界**（重跑循环末尾、确定还会再跑一趟时）出图，闸门 = 第 1 趟 ≥500 ms + 页数有涨 + 总次数 ≤8。实测代价 +326 ms / 冷编 7.4 s。
3. **事件语义**：进度用的部分 PDF **不能**覆盖权威 `pdf-updated`（否则「引用待更新」与页哈希缓存都会被污染）；需要一条独立事件 + 前端"编译中预览"的表现（页数在涨、目录/引用必然是 `??`）。→ **2026-09-16 已落地**：独立事件 `compile-preview`（§11.3 的时间线已真机验证）+ 前端"换字节不换身份"（工具条「编译中预览 · N 页」，见 `modules.md` §9.4 渲染契约）。
4. **切页边界**：前缀必须切在完整页（`eop`）上 —— 解析器已保证"半页回滚"，但要在产品路径上再断言一次。
5. **未测**：① 库形态下**内存里**取前缀并转换的端到端时延（本轮是"从文件前缀"量的，差的是 I/O 层那几十微秒）→ **2026-09-16 已测**：走产品路径（内存捕获表 → 合成 → 进程内转换）130–175 ms/次；② 与排版并发时的实际争抢（本轮是串行量的）→ **2026-09-16 已测并证否**：并发不可能（§11），改为趟边界；③ 用户在"页数逐渐变多"的中间态下的接受度（属产品判断，需要真机走查）。

---

## 11. 库形态的硬约束：引擎进程内全局锁 ⇒ **只能在趟边界出图**（2026-09-16）

### 11.1 约束本身（源码事实）

`tectonic_bridge_core::CoreBridgeLauncher::with_global_lock`（`lib.rs:306-326`）在**整个 C 引擎调用**期间持有
`static ENGINE_LOCK: Mutex<u8>`（`lib.rs:202`）。上游注释给了理由：C 侧错误上报基于 `setjmp/longjmp`，
跨 FFI 边界是 UB，所以全局只能有一个引擎在跑。

对本项目意味着：**同一个进程里，排版趟（`TexEngine`）与转换（`XdvipdfmxEngine`）不可能并行** —— 后来者只能排队。

### 11.2 实测（`crates/latteset-tectonic/examples/engine-lock-probe.rs`）

本机 24 逻辑核，125 页 XDV（4 594 056 B），release：

| 配置 | 每次 engine 耗时 | 墙钟 |
|---|---|---|
| 单线程 1 次 | **130 ms** | 132 ms |
| 4 线程并发 1 次/线程 | 132 / 265 / 398 / 530 ms（= 1×/2×/3×/4× 单位时间的排队） | **533 ms** |
| 单线程 4 次 | 132 / 133 / 133 / 134 ms | **541 ms** |

**4 个线程并发与"一个线程跑 4 次"墙钟相同（533 vs 541 ms）** ⇒ 零并行收益、完全串行。
（不是 CPU 争抢：24 核上 4 个转换不可能互相拖慢 4 倍。）

### 11.3 由此推翻的设计与替代

立项时写的"旁观线程按节拍看捕获表、够阈值就出图"**不成立**：

- 旁观线程只能在**趟与趟之间的空隙**里真正跑起来，而那时它的快照**按定义已经过期**；
- 实测那一版：帧落在终态 PDF 之前约 **0.1 s**（读者看不到任何东西），且它抢到锁时还会把权威转换往后推。

替代方案 = **趟边界出图**（已落地，`crates/latteset-tectonic/src/preview.rs`）：在重跑循环**末尾**（两个 `break`
之后 = 确定还会再跑一趟）同步调一次 `paint()`。此刻没有趟在跑 ⇒ 锁是空的，转换不排队、也不拖慢谁。

`bench/large` 冷编（125 页、热 format、本地 bundle）实测：

| 时刻 | 事件 |
|---|---|
| 1.72 s | 第 1 趟收尾（`typeset_ms=1718`）→ 前缀 4 571 180 B → **121 页**部分 PDF，转换 `engine_ms=130`、`open_ms=0` |
| 4.2 s | 第 2 趟收尾 → 125 页（158 069 → 163 158 B），转换 `engine_ms=175` |
| 7.4 s | 权威 PDF 落盘（163 201 B，125 页） |

⇒ 用户在 **1.9 s** 就看到 121 页、**4.2 s** 看到 125 页，而权威产物要等到 **7.4 s**：**提前约 5.5 s**；
代价 **+326 ms**（两次转换；单次 ≤175 ms ≤ 200 ms 预算）、总墙钟 **+4.4%**（7482 → 7783 ms，A/B 同夹具）。

### 11.4 代价与边界

- **单趟就收敛的编译拿不到中间帧**（一趟结束即终态）：这是锁的必然结果，不是可调参数；
- 想要**趟内**逐页出现，只能把转换搬到**另一个进程**（锁是进程内的）——那是一套新的进程编排（子进程要能开 bundle、要能与父进程对齐节拍），**本轮不做**，已登记为后续选项；
- 因此 ㉞ 的能力表述应收敛为"**长编译的趟间可见**"（多趟文档：冷编通常 2–3 趟），而不是"任意长编译都能逐页长出来"。
