...
| **㊺ 软件内新建文件/目录（候选，下一步做）** | 打开空目录或需要加文件时**不必跳出软件** | ① 空目录：新建 `main.tex` ⇒ 树出现 + 编辑器打开 + **能直接编译**（根探测重跑）；② 新建目录后可在其中新建文件；③ 重名 / 非法字符 / 项目外路径都有明确提示（不静默失败）；④ 真机验收：文件树右键与 `+` 入口都能用、`Esc` 取消不产生空文件 | ⬜ 未开始（2026-09-17 产品负责人点名入表，P=1.00） |
