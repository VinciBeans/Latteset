...
| ⑦a | 大纲重扫增量（⑦ 的第一个拆项） | 2026-09 | core `OutlineCache`（按内容指纹复用未变化文件的扫描结果，**内容每次仍重取**，外部改动不会被钉住）+ 前端只提交**变化过**的缓冲（`lastSent` 差分 + `open_paths` 淘汰）+ 刷新合并。**实测**（11 文件/436KB 工程）：全量重扫+全量提交 55.5ms → 编辑一个文件 **9.7ms** / 无编辑 **8.4ms**；真机端到端后端 `scans=1 reused=10` → `scans=0 reused=11`，前端两次分别只推 1 个与 0 个缓冲，大纲面板同步出现新标题。顺带修掉**入口路径未归一**（同一文件两种拼写 → 大纲重复项）与**并发刷新重复发送**（结构事件风暴实测连发 11 次）。契约见 [modules.md](../modules.md) §3.5/§12.2，数据与陷阱见 [p1-large-doc-editor-analysis.md](./p1-large-doc-editor-analysis.md) §3.3 |
...
|---|---|---|---|---|---|---|---|---|
| ⑦c | 多文件大项目（编辑器侧）：**先建夹具与口径** | 5 | 4 | 1 | 2 | **3.00** | **P0\*** | ⑦a 已完成（§1 基线）。⑦c 只先做"≥20 文件 × 大文件"夹具 + 测量口径（C1）；优化本身 C 待测——**没有数字就不投入** |
...
> P0 档当前只有 ⑦c，且它的 P0 身份来自"**先测量**"这一步（C1）：㉕ 首编超时、⑤ SyncTeX、⑥ CLI+MCP、⑦a 大纲增量均已完成并移入 §1 基线。

\* 已完成项里 ⑥ 的分数（2.00）与 ⑦a（2.25）都曾因 **W=3 / 拆分后 W=5** 的门槛差异进过不同的档：⑥ 的价值在"杠杆"（Agent 可驱动 + 顺带成为其他项的自动化验证手段），⑦a 的价值在"把每次编译都交一次的全量重扫变成增量"。**下一步排序**：⑦c（建夹具 + 出数）→ ⑧⑨ 语义层 → ⑩⑪㉖；⑦b（折叠增量）视 ⑦c 的数字再定。
...
⑦c/⑦b 见 §6.5｜⑧ 跨文件语义操作（**建议与 ⑨ 合并成"语义层"里程碑**，并与 ⑦a 落地的 include 图索引思路共用）｜⑩ 深色主题（Monaco + Monarch + pdf.js 反色需一起调）｜⑪ 预览滚动/缩放保持（建议与 ⑦c 共用夹具与口径）｜㉖ latexmkrc 交互（见 §6.1）｜⑨ LSP（先做 monaco-languageclient 技术验证）。

### 6.4 P2 / 后置

⑫ TinyTeX（C/V 最高，独立里程碑）｜⑬ AI（无一手需求证据，先观察）｜⑭ 拼写检查｜**㉙ 未存盘 overlay**｜⑮ 多窗口多项目｜⑯ 外部查看器｜⑰ 自动更新｜⑱ 代码签名。

### 6.5 ⑦ 大文档编辑器侧性能 —— 真机实测后的重排（2026-09）

完整数据、方法与代码走查见 **[p1-large-doc-editor-analysis.md](./p1-large-doc-editor-analysis.md)**。一句话结论：**竞品的架构病我们没有**——每击键净开销实测 0.02–0.05ms 且不随文档规模增长（5405 行/462KB 与 16213 行/1.36MB 同量级），"未闭合 `$` 触发全量重词法"与"每键两次 `getValue` 是大头"两个假设均被证伪。剩下的两处是我们**自己引入的耦合**：

| 拆项 | 实测 | 分数 | 状态 |
|---|---|---|---|
| **⑦a 大纲重扫增量**（`compile-status.success` → 提交**全部**缓冲 + Rust 重扫 include 图） | 修前 52ms@462KB → 91ms@1.36MB（每次编译一次）；**修后**（11 文件/436KB）9.7ms（编辑一个文件）/ 8.4ms（无编辑） | C2 V2 → **P 2.25** | ✅ **已完成（2026-09）**，见 §1 基线与 [实测分析](./p1-large-doc-editor-analysis.md) §3.3 |
| **⑦b 折叠提供者增量**（`getValue().split()` 全文档扫描） | 2.2ms@462KB → 3.7ms@1.36MB，折叠模型失效后重算一次 | C3 V2 → P 1.40 | 缓做（60Hz 帧预算内；144Hz + 多 MB 才明显） |
| **⑦c 多文件大项目**（调研 #4410 的原始症状：20 文件/数千页、打开 5–10 分钟） | **未测**（⑦a 验证时用过 11 文件/436KB 的临时夹具，已删；③ 的常驻夹具里 `multifile` 仅 94KB） | 建夹具 C1 → P 3.00；优化 C 待测 | **下一步**：先建"≥20 文件 × 大文件"夹具 + 口径，再据数决定 |
| ⑦d 每击键省一次 `getValue` | 收益 ~0.05ms | C1 V1 → P 4.50 | **明确不做**（分数好看、用户无感） |

**DoD 建议**：新增 `scripts/editor-report.mjs`（MCP 驱动真机，复用本次探针）——门槛 `每击键净开销 ≤0.5ms@1MB`、`折叠重算 ≤2ms@1MB`、`大纲往返 ≤20ms@1MB`、`无 >50ms longtask`。

**方法论教训（回写）**：① A/B 必须**顺序受控 + 预热丢弃**（首轮差 0.47ms，交替 3 轮后 0.02–0.05ms，单轮会高估 5–10 倍）；② 不能用 `slice()` 代理 `getValue()`（V8 SlicedString，O(1) 视图，实测 0.000ms）；③ MCP 的 `webview_keyboard type` **打不进 Monaco 0.56**（EditContext + `readonly` 的 IME textarea）——改用"动态 import 已加载模块 + `editor.trigger('keyboard','type')`"。
...
| **Batch 3 硬骨头** | ~~⑦a 大纲重扫增量~~（✅ 2026-09）→ ⑦c 多文件大项目（先建夹具出数）→ ⑧⑨ 语义层（合并） → ⑩ 深色主题 / ⑪ 预览状态保持 / ㉖ latexmkrc（**证据已就位，优先级应上调**） | 交互期无 >50ms 长任务；带 rc 的模板编译路径有结论；跨文件重命名可用；主题一致 |
