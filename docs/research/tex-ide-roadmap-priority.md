...
2. **悬停抖动**：鼠标扫过一行会路过多个公式 ⇒ 去抖 + 只在指针**静止**时触发；是否需要"悬停即编译"还是"悬停显示『预览』按钮/快捷键（TeXStudio 的 `Alt+P`）"，两者的**编译量差约一个量级**（实现成本相近）⇒ **P0 之后有了判定依据**：按首次冷启动耗时分级（>3 s 的项目只给按钮式），**待真机手感确认阈值**。
