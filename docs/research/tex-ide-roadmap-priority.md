...
|---|---|---|---|---|---|---|---|---|
| ⑦ | 大文档编辑器侧性能 → **⑦a 大纲重扫增量**（其余拆项见 §6.5） | 5 | 4 | 2 | 2 | **2.25** | **P0\*** | desktop 报告"最大可攻占缺口"；**2026-09 真机实测**：每击键开销不随规模增长（竞品的架构病我们已规避），真正随规模增长的是**每次编译成功后的全量大纲重扫**（52ms@462KB → 91ms@1.36MB）→ 重估 C，拆项见 §6.5 与 [实测分析](./p1-large-doc-editor-analysis.md) |
...
\* 已完成项里 ⑥ 的分数（2.00）仍是全表最高，但 **W=3** 不满足 P0 的 `W ≥ 4` 门槛——它的价值在"杠杆"（Agent 可驱动 + 顺带成为其他项的自动化验证手段），不在用户痛点权重。**下一步的排序**：⑦ 经真机实测（§6.5）拆分后，**⑦a 以 P=2.25 进入 P0 档**（W=5 ≥ 4 满足门槛），Batch 3 按 ⑦a → ⑦c → ⑧⑨ → ⑩⑪㉖ 推进。
...
⑦a/⑦c 见 §6.5｜⑦b 折叠增量（缓做）｜⑧ 跨文件语义操作（**建议与 ⑨ 合并成"语义层"里程碑**，并与 ⑦a 共用 include 图索引）｜⑩ 深色主题（Monaco + Monarch + pdf.js 反色需一起调）｜⑪ 预览滚动/缩放保持（建议与 ⑦c 共用夹具与口径）｜㉖ latexmkrc 交互（见 §6.1）｜⑨ LSP（先做 monaco-languageclient 技术验证）。
...
| 拆项 | 实测 | 分数 | 建议 |
|---|---|---|---|
| **⑦a 大纲重扫增量**（`compile-status.success` → 提交**全部**缓冲 + Rust 重扫 include 图） | **52ms@462KB → 91ms@1.36MB**，每次编译一次 | C2 V2 → **P 2.25**（P0\*） | **先做**；与 ⑧⑨ 共用 include 图索引 |
| **⑦b 折叠提供者增量**（`getValue().split()` 全文档扫描） | 2.2ms@462KB → 3.7ms@1.36MB，折叠模型失效后重算一次 | C3 V2 → P 1.40 | 缓做（60Hz 帧预算内；144Hz + 多 MB 才明显） |
| **⑦c 多文件大项目**（调研 #4410 的原始症状：20 文件/数千页、打开 5–10 分钟） | **未测**：本次只有单文件项目，③ 的夹具里 `multifile` 仅 94KB | 建夹具 C1，优化 C 待测 | 第二步：先建"≥20 文件 × 大文件"夹具 + 口径，再据数决定 |
...
| **Batch 3 硬骨头** | ⑦a 大纲重扫增量（**实测后升 P0**，见 §6.5）→ ⑦c 多文件大项目（先建夹具）→ ⑧⑨ 语义层（合并，与 ⑦a 共用 include 图索引） → ⑩ 深色主题 / ⑪ 预览状态保持 / ㉖ latexmkrc（**证据已就位，优先级应上调**） | 交互期无 >50ms 长任务；带 rc 的模板编译路径有结论；跨文件重命名可用；主题一致 |
...
| ⑦ 大文档编辑器侧性能 | **[p1-large-doc-editor-analysis.md](./p1-large-doc-editor-analysis.md)**（真机实测数据 + 代码走查 + 拆分建议）+ modules.md §12.1 |
...
13. **⑦ 的"大文档"只测了单文件**：462KB/5405 行与 1.36MB/16213 行两档，均为**单文件项目**；调研里 #4410 的症状是「20 个文件 / 近 7000 页」。多文件大项目的打开耗时、内存、树刷新、include 图解析规模**全未测**——⑦c 的第一步就是补这个夹具与数字（分析见 [p1-large-doc-editor-analysis.md](./p1-large-doc-editor-analysis.md) §5）。
14. **⑦ 的 W=5 与"我们是否真有这个病"是两件事**：实测显示竞品的卡顿来自架构（全文档同步计算），而我们在这些路径上已经规避（惰性词法 + O(1) 每击键处理 + Rust 侧解析）。W 保留（用户在意），但 C 必须按实测重估——这正是 §6.5 把 ⑦ 从"C=4 的大工程"拆成 ⑦a/⑦b/⑦c 的依据。