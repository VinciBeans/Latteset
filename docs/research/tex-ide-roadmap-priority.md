...
| ㉘ | **编辑期轻量出图 + 空闲收敛**（吸收自 §5）**——收益已验证：值得做** | 4 | 3 | 3 | 2 | **1.40** | P0 | 实测省 **40.2%**（6/6 档达阈值：tiny 31%/large 55%）；代价已量化（单趟的目录页码落后一趟：实测 29 vs 41）→ 落地需带"收敛中"提示 + 空闲收敛 + 首编仍走完整 latexmk |
...
**落地约束（必须，否则会变成"静默错误"）**：
1. **空闲后必须收敛**：停手 ~500ms 跑完整 latexmk（多趟 + bib + 索引），这是正确性的兜底；
2. **UI 必须显示"收敛中/引用待更新"**：连续打字期间目录可能长时间是旧的，不提示用户会以为是 bug；
3. **首次编译与手动「编译」仍走完整 latexmk**（避免用户主动要正确结果时拿到旧的）；
4. 预览侧无需改动（PDF 路径与拷贝逻辑一致）。

**预期收益（按 ③ 的口径换算端到端）**：`tiny` 2225→1719ms（FAIL→PASS）、`small-article` 3193→1920ms（FAIL→PASS）、`large` 4704→2961ms、`thesis` 3743→2416ms。

**未验证**：bib/biber 场景下单趟是否仍产出可用 PDF（本文档全部 fixture 均无引用写入，`thesis` 的 bibtex 未在编辑期触发）——落地前应补一组带 `\cite` 的编辑样本。
...
| **Batch 2 攻核心承诺** | ㉘ 编辑期轻量出图（先测收益）→ ⑤ SyncTeX 加固 → ⑥ CLI+MCP | 编辑延迟有量化改善或明确否决；SyncTeX 三组样本有记录；Agent 能驱动「改→编→验」 |
...
| ㉘㉚㉛ | [texpresso-live-rendering-roadmap.md](../texpresso-live-rendering-roadmap.md)（外部方案）+ 本文件 §5 | 本次新增 |
...
6. ~~**㉘ 的收益未测**~~ —— **已测：平均省 40.2%（6/6 档达阈值）→ 值得做**，代价（目录页码落后一趟）也已量化，见 §6.3。
