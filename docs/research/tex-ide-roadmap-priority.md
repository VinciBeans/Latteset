...
| 路线 | 做法 | 成本/证据 | 判定 |
|---|---|---|---|
| **① 独立小文档按需编译（主路）** | 把「项目导言区 + `\begin{document}` + 该公式 + `\end{document}`」装成一份小文档，在**独立项目根** `<项目>/tmp/snippet/` 编译，取第 1 页渲染 | **已实测两轮**。A（release、库形态、热 format）：最小片段文档第 1 趟 **79 ms**、真实片段 **204/200/198 ms**、同机全篇第 1 趟 **1716 ms**。**本轮 P0 补齐三档导言区的曲线与阶段分解**（§6.11.8）：**公式内容不构成成本**（空跑 ≈ 行内 ≈ 行间，差在噪声内）、**转换是固定大项**（145–409 ms）、**同一 snippet 目录重复悬停走 A 闸门只需 77 ms（轻）/332 ms（重）**、冷启动的悬崖来自**外部工具**（biber 8.6 s）而不是导言区大小 | **采用**（后端与三个坑的修法见 `git show 0ac930c`）。**设计修正**：snippet 目录**按公式保留**、不要照 A"每次清空重建" —— 保留才能吃到 **A 闸门**（页哈希相同⇒跳过转换）与**热 aux** |
...
- **验收判据**（先量后做的次序不能反）：
  ...
  3. 首次出图（缓存未命中）**按导言区分级**（P0 实测校准，§6.11.8）：**轻/中导言区 ≤ 400 ms**（实测 240/545 ms，含 145–300 ms 的转换）、**含 ctex/tikz 的重导言区 ≤ 900 ms**（实测 760 ms）；**同一公式的重复悬停 ≤ 400 ms**（实测 77/332 ms，走 A 闸门）；**首次冷启动若 > 3 s（外部工具介入，实测 biber 8.6 s）⇒ 该项目退回"悬停出按钮、点击才编译"**，且首测结果要落盘复用（别每次都探一遍）。
...
- **转换是固定大项**：145–409 ms，是 typeset 的 **1.3–2×**；与 ㉞ 记录的"进程内转换 130 ms"同量级（交叉一致 ✓）。预览要 PDF ⇒ 每次未命中都得付这一笔。⚠ **A 当年记的"转换 1 ms"与本轮（以及 ㉞ 的 130 ms）对不上** —— 疑为当时读错了阶段字段（`convert_ms` 是 `phase_ms.2 - phase_ms.1`，若那一趟没走转换分支就会是 0）；以**本轮实测 + ㉞ 的独立记录**交叉一致的 130–400 ms 为准。
