...
| ㉘ | **编辑期轻量出图 + 空闲收敛**（吸收自 §5，先测收益） | 4 | 3 | 3 | 2 | **1.40** | P0 | 多趟文档的编辑延迟（multifile 2.46s / thesis 3.14s，③ 实测）里有相当部分是多趟代价；**第 1 步是测量收益，<20% 则不做** |
...
### 6.3 ㉘ 编辑期轻量出图 + 空闲收敛（**吸收自 §5，先测收益**）

- **想法**：编辑期间只跑**单趟**（快出图，交叉引用可能显示为旧值），停手 ~500ms 后跑**完整 latexmk**（多趟 + bib + 索引）静默收敛——正是 TeXpresso 阶段 8 的"空闲触发而非每次编辑触发"。
- **第 1 步（必做，便宜）**：用 ③ 的基准测**单趟 xelatex vs 完整 latexmk** 的差值（六档各自）。**若收益 <20% 则不做**——别为一个测不出收益的机制引入"引用显示为 `??`"的状态不一致。
- **风险**：编辑期预览可能短暂显示未解析引用 → 需要明确的"收敛中"状态提示，否则用户会以为坏了。
- **依赖**：③（已完成）。
- **验证**：③ 的基准脚本量化 + MCP 真机端到端（`window.__previewLastReload`）。
...
6. **㉘ 的收益未测**：单趟 vs 完整 latexmk 的差值未测，故 P0 是"先测量再决定"，不是"确定要做"。
