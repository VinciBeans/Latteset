...
| ㊺ | **软件内新建文件/目录**（打开空目录或临时要加文件时，不必跳出去用资源管理器） | 3 | 2 | 3 | 2 | **1.00** | P1 | **需求原文（2026-09-17，产品负责人）**：打开空目录、或临时需要加文件时，现在必须**跳出软件**去建文件。现状：命令面有 `write`（`resolve_creatable_in_project` 允许**目标不存在** ⇒ 已经能建新文件）但**前端没有任何入口**；文件树只读。设计要点：① 文件树上的「新建文件/新建目录」入口（右键 + 工具栏 `+`）；② **文件名就地输入**（不弹系统对话框）；③ 新建后**刷新文件树并直接在编辑器打开**；④ 空目录特例：0 个候选根文件时 `open_project` 与 `compile` 都会拒（`未确定根文件（候选中 0 个）`）⇒ 新建第一个 `.tex` 后要能**重跑根探测**并接着编译；⑤ 路径校验沿用 D8（只能在项目内建）。判据：空目录里新建 `main.tex` → 树里出现 → 编辑器打开 → 直接能编译；重名/非法名给明确提示；建目录后可往里建文件。见 §6.13 |
...
| **㊺ 软件内新建文件/目录（候选，下一步做）** | 打开空目录或需要加文件时**不必跳出软件** | ① 空目录：新建 `main.tex` ⇒ 树出现 + 编辑器打开 + **能直接编译**（根探测重跑）；② 新建目录后可在其中新建文件；③ 重名 / 非法字符 / 项目外路径都有明确提示（不静默失败）；④ 真机验收：文件树右键与 `+` 入口都能用、`Esc` 取消不产生空文件 | 🟡 **实现完成（2026-09-17）、真机验收待补**：`FileTree` 标题栏 `＋` 就地输入名称 → `saveAll`（D8 校验）→ 刷新树 + 编辑器打开；**空目录特例**已处理（建完第一个 `.tex` 且项目还没有根文件时重跑 `openProject` 做根探测）；`npm run build` 通过 ✓ |
...
**落地状态（2026-09-17）**：**新建文件**已实现 —— `src/components/FileTree.vue`（标题栏 `＋` + 行内输入 + Enter 落盘 / Esc 取消 + 错误提示留白不静默）、复用 `ipc.saveAll`（目标可不存在 ⇒ 后端无需改动）、成功后 `refreshTree()` + `editor.openFile()`，并在"项目还没有根文件"时重跑 `openProject`（空目录那条路）；`npm run build`（vue-tsc + vite build）通过 ✓。**待办**：① 真机验收（空目录建 `main.tex` ⇒ 树出现 + 编辑器打开 + **能直接编译**；重名/非法名提示；Esc 不留空文件）；② 新建**目录**（`FileSystem` trait 目前没有建目录入口，要在 infra 加一个，再照同样范式接命令面与前端）；③ 右键在选中目录里建（v1 走"名称里带 `chapters/` 落到已有子目录"）。
