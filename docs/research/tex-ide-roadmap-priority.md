...
**快速通道规则**：**成本 ≤2 且独立可交付**的小项随时可插队（不占批次位次）。当前通道：**㊱㊲㊳**（2026-09-16 入表：SyncTeX 失败重试 / 首屏让位 / 保存→开编往返，C≤2、彼此独立，见 §6.9.1）；㉜ 已收口（§1）；㉝（P2，未指定）仍可随时插队。
...
| ㊱ | **SyncTeX 失败查询的重试语义**（失败时白等 0.6 s；inverse 3.1 s 成因待定位） | 2 | 1 | 1 | 2 | **1.00** | P1 | 实测（release、`bench/large` 125 页、`.synctex.gz` 104 KB）：正常查询 **16–26 ms**（含 ~10 ms 进程启动 ⇒ 解析本身 **~7 ms**）；**源文件不在同步数据里** → **668 ms**；**完全没有同步数据** → forward **650 ms** / inverse **3127–3156 ms**（是 forward 的 5 倍，疑似两层退避叠加 —— **定位它本身就是本项的一半**）。根因：`infra/synctex.rs` 的 `with_retry` 把**所有**错误都退避重试，含"没有这个源文件"这种**确定性**失败。修法：只对瞬时 I/O 错误退避，"无映射/文件缺失"立刻失败。**C=1 ⇒ 走快速通道**（§6.9.1） |
...
| ㊳ | **保存→开编链路的往返成本**（先量后定；**别动** 500 ms 防抖） | 2 | 1 | 1 | 2 | **1.00** | P1 | 澄清一条误判：`debounce_ms`（当前设置 **500 ms**）是**刻意的合并窗口**（一次编辑常伴随多处写盘），不该删；可省的只是"自家写盘也要绕一圈 watcher（FS 事件 → 忽略过滤 → 排编译）"的往返，量级**未测**（估计毫秒～几十毫秒）。只有 ≥100 ms 才值得做"自写盘快速通道"，且必须保证外部编辑仍触发、不重复触发。**C=1 ⇒ 走快速通道**（§6.9.1） |
...
| **Batch 5 并行与"白等"** | 快速通道 **㊱㊲㊳** → **㊴ 子进程编译通道**（→ **㊶ 分章并行草稿**）→ ㊵ | ㊱：失败查询 ≤100 ms，且 inverse 的 5× 成因定位并消除；瞬时错误（编译中同步文件被重写）仍能按退避重试成功（㉒ 行为不回归）。㊲：125 页文档编译后，首屏可见时间不再包含索引耗时（口径：`__previewLastReload` + 首屏 canvas 完成时间戳）；草案层可用性不降。㊳：先出往返实测数，≥100 ms 才做。㊴：主编译墙钟增量 ≤10%（真机 ≥8 核）、**低核机器（≤4 核）默认不开**、缓存零互踩、进程杀干净无残留。㊶：分章草稿的页码/编号与全篇一致（先做页哈希对拍） | ⬜ 未开始（2026-09-16 入表；依据与实测见 §6.9） |
...
**㊱ SyncTeX 失败查询的重试语义**（C=1）

实测（release、`bench/large` 125 页、`.synctex.gz` **104 KB**）：

| 情形 | 墙钟 |
|---|---|
| 有映射的行（第 100 / 1500 行） | **16–26 ms**（含 ~10 ms 进程启动 ⇒ 解析本身 **~7 ms**） |
| 空行 / `\documentclass`（第 7 / 1 行） | 16 ms（解析器**就近回落**，不算失败） |
| **源文件不在同步数据里**（`nosuch.tex`） | **668 ms** |
| **完全没有同步数据**（`tmp/` 被清空） | forward **650 ms** / inverse **3127–3156 ms** |

- **根因**：`crates/latteset-infra/src/synctex.rs` 的 `with_retry(&RETRY_BACKOFF, …)` 把**所有**错误都退避重试 —— 包括"同步数据里没有这个源文件"这种**确定性**失败（3 次 × (load+parse) + 100/200/300 ms 睡眠）。
- **inverse 的 5 倍是待查项**：3 次退避只有 600 ms，实测 3.1 s ⇒ 疑似**两层退避叠加**（provider × 调用层）。定位它本身就是本项的一半。
- **验收判据**：失败查询 ≤100 ms；瞬时错误（编译中 `.synctex.gz` 被重写）仍按退避重试且真能成功（㉒ 的既有行为不回归）。
- **复现命令**：`latteset-cli --project <夹具> forward <文件> <行>`（CLI 每次都是**新进程**，正好把"每次点击都要重新解析"这点也量进去）。
...
- **澄清一条误判**：`debounce_ms`（当前设置 **500 ms**）是**刻意的合并窗口**（一次编辑常伴随多处写盘），**不该删**。
- 可省的只是"自家写盘也要绕一圈 watcher（FS 事件 → 忽略过滤 → 排编译）"的往返，量级**未测**（估计毫秒～几十毫秒）。
- **判据**：先测出这段往返（日志时间戳：`写完盘` → `开始编译` 减去 `debounce_ms`）；只有 ≥100 ms 才值得做"自写盘快速通道"，且必须保证**外部编辑仍触发**、**不重复触发**。
...
10″. **㊳ 的"保存→开编往返"未测**：需要一组日志时间戳，把 FS 事件 + 忽略过滤的往返从 `debounce_ms`（500 ms，刻意设计）里剥出来；<100 ms 就不做（§6.9.1）。
10‴. **㊱ 的 inverse 3.1 s 成因未定位**：3 次退避只有 600 ms，实测却是 forward 的 5 倍 ⇒ 疑似**两层退避叠加**（provider × 调用层），未证实（§6.9.1）。
