...
- **切片计划**（照 ㉞ 的做法：每片自带可验证判据，片内不留半个功能）：
  ...
  2. **切片 2（命令面 + 隔离，可 headless 断言）**：`compile_math` 命令（公式文本进 → 单页 PDF/PNG 出）+ 独立项目根 + **按公式保留 snippet 目录**（吃 A 闸门与热 aux）+ 首次冷启动计时与分级退让标志；判据 = 同一公式两次调用的耗时（§6.11.8 的 77/332 ms 档位）、坏公式给原因、权威 `main.pdf`/`tmp/main.*` 逐字节不变（照 ㊷ 的 A/B）。
  3. **切片 3（前端悬停浮层 + 真机验证）**：Monaco `registerHoverProvider` + 去抖 + 两级复用 + 卡片（失败不留白）；判据 = 真机悬停延迟、主编译墙钟增量 ≤5%、主编译期间不发起（日志）、可观测钩子断言。
- **验收判据**（先量后做的次序不能反）：
