...
| **㊺ 软件内新建文件/目录（候选，下一步做）** | 打开空目录或需要加文件时**不必跳出软件** | ① 空目录：新建 `main.tex` ⇒ 树出现 + 编辑器打开 + **能直接编译**（根探测重跑）；② 新建目录后可在其中新建文件；③ 重名 / 非法字符 / 项目外路径都有明确提示（不静默失败）；④ 真机验收：文件树右键与 `+` 入口都能用、`Esc` 取消不产生空文件 | 🟡 **实现完成 + 真机验收部分通过（2026-09-17）**：`FileTree` 标题栏 `＋` 就地输入名称 → `saveAll`（D8 校验）→ 刷新树 + 编辑器打开；**空目录特例**已处理（建完第一个 `.tex` 且项目还没有根文件时重跑 `openProject` 做根探测）；`npm run build` 通过 ✓。**真机**：空目录里建 `main.tex` ⇒ 落盘/树/编辑器打开都 ✓，但**根探测认不出空文件**（候选 0）⇒ "建完就能编"待修（最小骨架 / 保存后重探测） |
