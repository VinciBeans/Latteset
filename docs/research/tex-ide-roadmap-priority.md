...
**快速通道规则**：**成本 ≤2 且独立可交付**的小项随时可插队（不占批次位次）。当前通道：**㊲**（2026-09-16 入表：首屏让位，C=2，见 §6.9.1）；**㊱ 已收口**（2026-09-17，见 §1）；㊳ 已**定量否决**（往返 ≈7 ms，见 §1）；㉜ 已收口（§1）；㉝（P2，未指定）仍可随时插队。
...
| ㊷ | **库形态下 SyncTeX 双向定位不可用**（同步数据里没有真实源文件名） | 4 | 1 | 3 | 2 | **1.00** | P1 | 真机证据（2026-09-17，同夹具两形态对照）：库形态的 `.synctex.gz` 里 **`Input:1:texput`、`Input:2..8` 全空** ⇒ `forward` 必然报"同步数据里没有这个源文件"（**静默失效**：状态栏显示"库内嵌"但不提示定位不可用）；子进程档是 `Input:1:E:\…\main.tex`、`forward` 返回 `{"ok":true,"page":8}` ✓。根因：库形态**内存直喂主文件**（XeTeX 的 jobname 落成默认 `texput`）。C=3 是 **[推断]**（修法要么让主输入带真实路径、要么在 I/O 层补 jobname/Input 记录，需先勘察 `io.rs` 主输入路径）。见 §6.10；这是 ⑤ SyncTeX 在**库内嵌档**的功能缺口，不是性能问题 |
| ㊲ | **首屏让位：文本行索引不再挡首屏** | 3 | 1 | 2 | 2 | **1.00** | P1 | 现状（`PreviewPane.load()`）：先 `void buildLineIndex(true)`（**遍历 1..numPages**），再 `renderNearViewport()`，两者**共用同一个 pdf.js worker** ⇒ 索引的 `getTextContent()` 排在首屏渲染前面。实测 28 页 **164–173 ms**；125 页按页线性外推 **≈0.7–0.8 s**（**外推，第一件事是实测它**）。两步：①索引挪到首屏渲染之后（零风险）；②仍不够再上第二个 worker/文档实例。**C=2 ⇒ 走快速通道**（§6.9.1） |
...
| ㊴ | **子进程编译通道**（草稿/辅助编译与主编译**并行**；同时解锁 ㉞ 的"趟内逐页"） | 4 | 3 | 5 | 4 | **0.78** | P2 | 唯一的大杠杆：引擎锁是**进程内**的，跨进程不受它管。实测两进程并发各 **1716 → 2248 ms**（≈+30%，**真并行**；24 逻辑核），小文档草稿 ~**200 ms**（release）。前置三件套：`CompileRequest` 的 tmp/输出覆盖、缓存**只读**（tectonic 缓存无进程间锁）、生命周期与树杀。详见 §6.9.2 |
...
| **Batch 5 并行与"白等"** | 快速通道 **㊲** → **㊴ 子进程编译通道**（→ **㊶ 分章并行草稿**）→ ㊵ | ㊱：✅ **已完成（2026-09-17）**——失败查询 **668 → 14 ms**、无同步数据 **645 → 10 ms**、inverse **3127 → 11 ms**，瞬时竞争仍退避成功（㉒ 不回归），见 §1/§6.9.1。㊲：125 页文档编译后，首屏可见时间不再包含索引耗时（口径：`__previewLastReload` + 首屏 canvas 完成时间戳）；草案层可用性不降。㊳：✅ **已定量否决**（往返 ≈7 ms，不做，见 §1）。㊴：主编译墙钟增量 ≤10%（真机 ≥8 核）、**低核机器（≤4 核）默认不开**、缓存零互踩、进程杀干净无残留。㊶：分章草稿的页码/编号与全篇一致（先做页哈希对拍） | 🟡 入表 2026-09-16；**㊱ 已完成**、**㊳ 已定量否决**，余项未开始（依据与实测见 §6.9） |
| **㊷ 库形态 SyncTeX 修复** | 让**库内嵌档**的同步数据带上真实源文件名（正向/反向定位都可用） | 库形态编译后 `.synctex.gz` 里有 `Input:N:<真实路径>`（不是 `texput`/空）；`forward` / `inverse` 与**子进程档逐点对拍**一致（`scripts/synctex-report.mjs` 三组样本）；"库内嵌"不再等于"定位不可用"（若短期修不动，至少要**如实提示**，不得静默失效） | ⬜ 未开始（2026-09-17 入表；真机证据见 §6.10） |
...
**结论速览**：真正的大杠杆只有一个 —— **㊴ 把编译搬进另一个进程**（引擎锁是**进程内**的）；其余都是 0.1–0.8 s 量级的小项（㊱㊲㊳ 走快速通道）。另有**四条我原本怀疑、被实测否掉**的（§6.9.3），一并回写，免得后人重复投入。
...
- 现状（`src/components/PreviewPane.vue`）：`load()` 里 `void buildLineIndex(true)` 先发，`renderNearViewport()` 后发，两者**共用同一个 pdf.js worker** ⇒ 索引的 `getTextContent()`（**遍历 1..numPages**）排在首屏渲染前面。
- 实测/外推：28 页 **164–173 ms**（[realtime-preview-cost.md](./realtime-preview-cost.md) §6）；125 页按页线性外推 **≈0.7–0.8 s**（**外推，第一件事是把它实测出来**）。
- 两步：① 把索引挪到首屏渲染之后（首个 render 完成/idle 再建）—— 零风险，先做；② 仍不够再上第二个 worker/文档实例（代价：多一份 PDF 解析内存）。
- **验收判据**：125 页文档编译后，首屏（第 1 页 canvas 画完）时间戳不再包含索引耗时；草案层的"改动行定位"可用性不降（索引仍要建，只是晚一点）。
...
### 6.10 ㊷ 库形态下 SyncTeX 双向定位不可用（2026-09-17 真机发现）

**一句话**：库内嵌档编译出的 `.synctex.gz` **没有真实的源文件名**（`Input:1:texput`，`Input:2..8` 全空）⇒ **正向定位（源码 → PDF）必然失败**；反向定位同样拿不到可打开的源码。而状态栏只显示"库内嵌"，**不提示定位不可用** —— 属于**静默失效**。

**怎么发现的**：做 ㊱ 时把"失败查询"的**载荷**打了出来（此前只看墙钟），于是看到 `forward` 报的是 `Parse`："同步数据里没有这个源文件（…\main.tex）" ✗；顺着解压 `.synctex.gz` 就看到了空 Input。

**两形态对照**（同夹具 `bench/large`、release、同一台机器）：

| | `Input:1` 记录 | `forward` 第 100 行 |
|---|---|---|
| **子进程档**（`tectonic.exe --synctex`，`LATTESET_TECTONIC_LIB=0`） | `Input:1:E:\Works\…\large\main.tex` ✓ | `{"ok":true,"page":8,"x":133.77,"y":204.47}` ✓ |
| **库形态**（`LATTESET_TECTONIC_LIB=1`） | `Input:1:texput`、`Input:2..8` **空** ✗ | ✗ `Parse`：同步数据里没有这个源文件 |

**根因（已定位到形态，未定位到代码行）**：库形态把**主文件内存直喂**引擎（[modules.md](../modules.md) §2.7「输入四层」的第一层）⇒ XeTeX 眼里的作业名是默认的 `texput`，`Input:` 记录因此没有真实路径；子进程档是命令行给文件路径 ⇒ 记录正常。
**待勘察**：`crates/latteset-tectonic/src/{io,runner}.rs` 的主输入喂法 —— 能不能带真实路径 / 能不能设 jobname / 能否在 I/O 层补 Input 记录。这正是 §3 里 C=3 那个 **[推断]** 的来处。

**为什么值得优先**：⑤ SyncTeX（源码 ↔ PDF 双向定位）是本产品的头牌能力之一，而它在**库内嵌档**（"零预装可用"的那条路）下完全不可用、UI 还不提示；用户视角就是"点了没反应"。

**验收判据**（同 §4 的 ㊷ 批次行）：`Input:` 记录里有真实路径；两形态的 `forward`/`inverse` **逐点对拍一致**（`scripts/synctex-report.mjs` 三组样本）；若短期修不动，就**如实降级提示**（状态栏/点击时说明"库内嵌档不支持定位"），不得静默失效。

**与 ㊱ 的关系**：㊱ 修的是"失败要**快**"，本项修的是"**不该**失败" —— 别把两者混为一谈。
...
10′. **㊲ 的"索引挡首屏"只有外推**：28 页 **164–173 ms** 是实测，125 页 **≈0.7–0.8 s** 是**按页线性外推**（§6.9.1）。要先在 125 页文档上把"首屏 canvas 完成时间戳 − 索引耗时"实测出来，再决定走①还是②。
