...
| 阶段 2 子进程 + 流式输出 + 时间片 | 🟡 **子进程 ✅ / 时间片 不需要 / 流式输出：缺管道但可达**（2026-09 专项实测，见 §5.8）：我们的 runner 把引擎 stdout/stderr **整个丢弃**（`Stdio::null()`），所以既没有编译期进度也没有实时错误；三条进度通道实测可行（stdout 页码标记 / 日志尾随 / XDV 页索引，时间线一致），且"页前缀 + 合成 postamble → 部分 PDF"已被做通（`xdvipdfmx` 接受、页数正确、首页渲染一致，成本 0.65–0.94s/次）。时间片结构性不需要：32s 编译期间打字 4.42ms/键、零 >50ms longtask |
...
| **编译期页完成度进度** | 阶段 3 的页索引（去掉渲染） | §5.7 / §10-B.8：读 `tmp/*.xdv` 出「已排版 N 页」，C1，未接线 |
| **页级变更判定** | 同上（页哈希差分） | ⑪ 的候选判定输入（§6.4）：保证不漏，但重排时会覆盖大量页 |
| **引擎 stdout 管道 + 部分 PDF** | 阶段 2（子进程 + 流式输出） | §5.8：`Stdio::null()` → `piped()` 出**编译期进度 + 实时错误**（C1）；页前缀 + 合成 postamble 出**部分 PDF**（可行，长文档再评估） |
...
### 5.8 阶段 2（引擎子进程 + 流式输出 + 时间片）可行性 —— 2026-09 专项

完整实测见 **[stage2-streaming-feasibility.md](./stage2-streaming-feasibility.md)**。要点：

- **子进程 ✅ 早有**（`infra::runner`，含超时树杀）；**唯一缺口是输出通道**：`runner.rs:110-111` 的 `Stdio::null()` 把引擎 stdout/stderr 全丢了 → 编译期既没有进度也没有实时错误。
- **时间片 ➖ 不需要**：引擎是独立 OS 进程。实测 32s Full 编译期间打字 **4.42ms/键、p95 7.2ms、零 >50ms longtask**（空闲对照 4.33ms / p95 5.4ms）。
- **流式输出 🟡 可达**：三条进度通道实测时间线一致（stdout `[N]` 标记 / 日志尾随 / XDV 完整页，首个非零都在 ~696ms = 编译 48%）；日志尾随的坑是"原地截断，必须按编译起点重置偏移"。
- **"部分 PDF" 做通了**（这是验收里"输出一产生就显示"的唯一现实路径）：页前缀 + **从零合成 postamble** → `xdvipdfmx` 接受 → 页数 = 前缀页数（12/30/60/90/121 全对）、首页渲染与完整文件一致（仅字体编号不同）。成本：构建 0–4ms + 转换 **0.65–0.94s/次**（与页数几乎无关）。坑：字体定义必须同时收 `define_native_font`(252) **和** 经典 `fnt_def`(243–246)，`post_post` 回指指针必须改。
...
8′. **阶段 2 的两条开口（2026-09 专项）**：① runner 的 stdout 管道未接（`Stdio::null()`）——接上就能拿到**编译期页进度 + 边编译边报错**，C1，且 core 已有 `pages_typeset()`；② "渐进部分 PDF"只做到**可行性**（能造出来），**端到端体感未评估**（滚动位置稳定性、pdf.js 频繁重载抖动、额外 `xdvipdfmx` 对编译时长的影响都未测），且只对 >5s 的长编译有意义。
