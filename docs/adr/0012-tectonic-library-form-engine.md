...
- **形态的默认值：已裁决 —— 保持「子进程」（2026-09-15，产品负责人）**。库形态**不跑外部进程** ⇒ biber / makeindex / glossaries 的文档在它下面会直接失败（检出即 `warn!`、`kind` 退回 `Quick`）。把默认切过去就等于让这类文档开箱即败；若要切，得先决定"检出外部工具时退回子进程"的语义 —— 而那与 D1「不静默回退」有张力。⇒ 现状：**编译进来的构建里两种形态都在，由用户在设置面选**（默认子进程）。

## 补充：库形态下「草稿」是判定结果，不是固定档（2026-09-15）

ADR-0005 与 ㉘ 把编辑触发定为 `Quick`（单趟、目录/引用落后一趟），再由前端停手 2 s 补一次 `Full` 收敛。**这条产品语义只对子进程档成立**：库形态的 runner 手里有重跑循环与中间产物指纹（`.aux`/`.toc`/`.bbl` 摘要），"这一趟落没落后"因此是**可判定**的。

- **决策**：编辑触发的请求在库形态下同样跑重跑循环（含 bib 趟）；**预算内**（3 趟 ∧ 第 1 趟 <1 s ∧ 累计 <2 s，末值与前端 `DELAY_MS` 同值）收敛就按**实际**结果报 `Full` ⇒ 前端 `draft=false`：不亮「引用待更新」，空闲收敛也不会再排一次。预算用尽、或文档要库形态跑不了的外部工具时，停在草稿态 —— **行为与 ㉘ 原样一致**。
- **依据（release 实测，§6.5）**：28 页带 bib 夹具已收敛 **1 趟 475–487 ms 报 `Full`**、真实改动 **2 趟 1115 ms 报 `Full`**；旧路径到正确结果 ≈475 + 2000 + 1115 ms。125 页 / 462 KB 夹具真实改动超闸门 ⇒ **1 趟 1847 ms 报 `Quick`**（退化档，与旧行为一致）。
- **影响**：子进程档与 `CompileKind` 的既有语义**不改**（`Quick` 仍然是"编辑触发"这个请求强度）；变的是库形态**如何上报实际做到的强度** —— 这正是 V-04「按实际做到的事报」的延伸。前端不需要改：`draft` 本来就取终态的 `outcome.kind`（`scheduler/actor.rs`），空闲收敛本来就只看 `draft`。
- **代价与已知边界**：阈值按**发布档**标定（`dev` profile 的 C 引擎按 `-O0` 编译，同一趟慢 ≈4.6×）⇒ `tauri dev` 下会更早退化成旧行为；阈值是否按机器自适应**未测**。
