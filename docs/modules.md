...
| 21 | **Tectonic 库形态（路径 B）的三项能力缺口** | 见 §2.7：**bib 趟**（无 `tectonic_engine_bibtex` 依赖 ⇒ 带 `\cite` 的文档引用可能不解析，检出时 `warn!`）、**页哈希**（XDV 只在内存 ⇒ `Success.page_hashes` 空表 ⇒ 下游保守全量刷新，A/B/C 页级复用在该档不可用）、**多趟收敛**（只跑一趟 ⇒ `kind` 报 `Quick`）。三者都**显式登记为不支持**，不是静默跳过；补齐顺序 = bib → 收敛 → 页哈希 |
