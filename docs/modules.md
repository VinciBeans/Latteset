...
| 装配 | **两个入口各一处**（GUI 与 headless）：`src-tauri/src/lib.rs` 与 `crates/latteset-server/src/lib.rs` 的 `build_runner()`，口径相同 —— `tectonic-lib` 特性（默认关）+ 运行期 `LATTESET_TECTONIC_LIB=1`（D1：失败不回退）。deps 侧 `src-tauri/Cargo.toml` 与 `crates/latteset-server/Cargo.toml` 都是 optional。GUI 侧装配的是 `SwitchableRunner`（每趟编译读一次全局设置 ⇒ 改完即生效） |
