...
| 21 | **Tectonic 库形态（路径 B）剩余的能力缺口** | 见 §2.7：**页哈希**（XDV 只在内存 ⇒ `Success.page_hashes` 空表 ⇒ 下游保守全量刷新，A/B/C 页级复用在该档不可用）、**外部工具类**（biber / makeindex / glossaries —— 库形态不跑外部进程；检出时 `warn!` 且 `kind` 退回 `Quick`，由 ㉘ 的「引用待更新」兜底）。**bib 趟与多趟收敛已于 2026-09-15 补齐**，不再属于缺口。两者都**显式登记为不支持**，不是静默跳过 |
