...
前端过滤自己刚自动保存的文件（editorStore 比对 path，避免重载光标跳动）；打开且不脏 → 静默重载；打开且脏 → 保留本地缓冲 + 状态栏提示（冲突对话框后置）。**重载写回 model 时必须屏蔽这次内容变更事件**：Monaco 的 `setValue` 会同步触发它，不屏蔽就会被当成用户编辑（tab 误判为脏 → 自动保存把同一份内容写回磁盘，后面的第二条 `files-changed` 再误报「外部修改」）。两条分支都以「磁盘内容 == 缓冲内容 ⇒ 不算冲突」收口。
