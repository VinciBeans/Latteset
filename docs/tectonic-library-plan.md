...
**仍未判 / 未测的**（**不得当成立**）：t5 的 U1–U5（§13）；构建代价三项（§8.3）；"空缓存 + 有网"的真实 bundle 下载行为与文案（U3）；常驻档的内存与并发（LBR-7，随 P4 结案一并搁置）；P5 的提前量阈值（LIB-3d）；**编辑触发档收敛预算的机器自适应**（§6.5：阈值现为常数、按发布档标定，其它机器/CI 上"一趟耗时"的分布未测）。
