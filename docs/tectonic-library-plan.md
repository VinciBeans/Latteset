...
> 文档状态：**已交付（t6 attempt 1）**。依赖 t1/t2/t3/t4/t5 全部已回数并落地：t1 → §2/§4 的现状与契约面；t2 → §3.1/§3.3/§3.4 的 API 面；t3 → §4/§5 的 I/O·确定性·bundle/缓存；t4 → §3.1.3/§6 P1/§8.3 的构建代价；t5 → §0/§6 P4–P6/§7/§8.2 的判据实测。t7（运行时对比）已回数（"库形态对击键路径无可证实改善"）⇒ §8.2 的「相对子进程对比列」按 §6.1 的 release 口径看。
> **实施状态见下一节**（按 §6 闸门逐条；只写已核实的事实，未核实的留在 §13 的未测清单里）。
...
> 约束：本文档写方案与判据；实施状态以「实施现状」节的核实结论为准。

## 实施现状（§6 闸门的当前情况）

| 阶段 | 状态 | 证据 | 下一步 |
|---|---|---|---|
| **P0** `-C` 条件化与首次获取引导 | **已落地** | `tectonic_command` 按 `BundlePolicy` 判定，仅 `CachedOnly` 加 `-C`（`crates/latteset-infra/src/runner.rs:338-341`），argv 断言在 `:925`；设置面的首编提示与该行为一致 | 无 |
| **P1** 依赖引入与最小可编译 | **落盘部分已落地**；**构建代价三项仍是 `[未测到]`** | crate 落点与 `default-members` 见 §3.2；干净机器的冷/热墙钟、峰值内存、体积增量仍未测（§8.3） | 补测受外部前置阻塞：vcpkg + cmake + pkg-config + nasm + 网络（§3.1.3） |
| **P2** 骨架：库形态 runner + 内存产物 | **已落地** | §6.1；可复核入口 `scripts/validate-pdf.mjs` | 无 |
| **P3** 自定义 I/O + 缓存隔离 + 离线 | **已落地** | §6.1（三类 bundle 失败各给文案、项目目录无 `.fmt`、format 落产品缓存） | 无 |
| **P4** 常驻进程与 format 复用（J1） | **已判决：不成立 ⇒ 按回退点不投入**（阶段结案） | §6.4：进程地板 9.3 ms；重档 **+31%** 且第 3 轮起退化 | 改投 §6.4 ④ 的两条杠杆（见下表） |
| **P5** 编译中页事件（J2） | **未开工**；可行性已实测成立，I/O 层的逐块钩子已预留 | `crates/latteset-tectonic/src/io.rs:76` 的逐块通知**目前没有产品侧消费者**；`XdvParser` 只出现在 `examples/xdvscan.rs` | 接 `XdvParser` → `CompileProgress`（提前量阈值 LIB-3d 仍 `[待补测]`） |
...
| **构建变体（发布路径）** | **已落地**：开发默认不含库形态（主产物零原生依赖），**发布构建含**（`npm run lib:build`）⇒ 用户装到的包两种形态都能选 | 前置与环境的唯一落点 `scripts/with-tectonic-lib.ps1`；CI 的 `build-windows-tectonic-lib` 档（版本 tag / 手动 dispatch 触发）。实测：`lib:build` 出 NSIS 16.24 MB / MSI 22.61 MB；默认档 7.19 MB（§8.3 的 A/B） | 形态**默认值已裁决 = 保持子进程**（ADR-0012 补充节：库形态不跑外部进程 ⇒ 默认切过去会让 biber/makeindex 文档开箱即败；要切得先定"检出外部工具退回子进程"的语义，与 D1 有张力） |
| **流式出图（部分 PDF）** | **未开工**；可行性 2026-09-15 重评（[DVI 报告 §10](./research/dvi-preview-feasibility.md)） | 合成 postamble **9–12 ms** + **进程内** `XdvipdfmxEngine` **92–131 ms/次**（31→125 页前缀，页数全对；外部进程同档 497–564 ms）；价值窗口 = 冷 Full 125 页 **7601 ms** 里"排版结束→出 PDF"那 5736 ms；体感已测：5 次背靠背重载 **0 long task**、滚动位置 1797 帧零偏移 | **已立项 roadmap ㉞**（[§6.6](./research/tex-ide-roadmap-priority.md)）：最窄切片 = 内存前缀钩子（`io.rs` 已预留、无消费者）→ Rust 合成器 → **独立**进度事件 → 阈值与限流；前置约束见该节"非目标/未测" |

**P4 判决后改投的杠杆**（§6.4 ④ 点名的"有数量级差别"项 + 2026-09-15 新增的一条）：

| 杠杆 | 状态 | 实测 |
|---|---|---|
| bib 输入未变时跳过 bibtex **及它强制的那趟重跑** | **已落地** | 单文件 `proj-cite` −57%（941 → 400 ms）；28 页多文件 `proj-26` −60%（1630 → 646 ms）。见 §6.4 ④-a |
...
| G5 | 判定并记录 J1/J2/J3 三判据的成立/不成立，写进门禁编号（§7） | `roadmap:246`（行号随 §6 拆分已漂移 ⇒ 现读作 [roadmap §6.5](./research/roadmap-item-specs.md)）、t1 §3 |

### 1.2 非目标（本轮明确不做）

| # | 非目标 | 理由 / 出处 |
|---|---|---|
| N1 | 不替换子进程形态 | `roadmap §6.5`（拆分后见 [roadmap-item-specs.md](./research/roadmap-item-specs.md)）「官方 MSVC 二进制**只能**当子进程用」；子进程路径是 TL-less 与打包分发的既成出路 |
| N2 | 不改默认引擎 | `crates/latteset-core/src/settings/model.rs:40` 默认 `XeLaTeX`；t1 §1.1 T-07。**⚠ 已被 ADR-0014 修订 1 覆盖（2026-09-17）：默认引擎已切 Tectonic**（`settings/model.rs:159-164`，单测 `defaults_match_design`）；本行只对当时的库形态立项范围成立 |
| N3 | 不重造引擎 / 不 fork | [roadmap §7](./research/tex-ide-roadmap-priority.md)「明确不做」表（自建/改造 TeX 引擎）；原文此行号 `tex-ide-roadmap-priority.md:263` 已漂移 |
...
| 项 | 取值 | 理由 |
|---|---|---|
| 默认 | **子进程形态**（形态位缺省） | N1；默认引擎本就不是 Tectonic —— **⚠ 已被 ADR-0014 修订 1 覆盖（2026-09-17）：默认引擎现为 Tectonic（子进程档）**；形态位缺省仍是子进程，库形态要构建时开特性 |
...
| **准入判决（t5 后 → 2026-09-15 结案）** | **判决：不成立**（§6.4）。t5 只判了三条判据（①②③），没测「常驻 vs 每次起进程」的对照 ⇒ 当时记 `[仍无法判定]`、闸门指向 t7；t7 交付复核命令后本阶段实测结案：进程地板仅 **9.3 ms**，轻档净收益 −26 ms、**重档 +31%**（第 3 轮起退化）⇒ 达不到「< 子进程地板 × 0.8 且省 ≥100 ms」 |
| 入口条件 | ——（阶段已结案，不再有入口条件） |
...
> **证据落点约定**：复核脚本与结论进 **`scripts/`**（脚本）或 **`docs/research/tectonic-lib-evidence.md`**（结论与原始摘要，**入库**）；`test_file/research-tectonic-lib/**` 只作为**开发期原始证据**，不作为可复核入口（换机器/清仓即丢）。**该文件已于 2026-09 落盘**（本文 §6.1–§6.5 已迁入其中）。

### §6.1–§6.5 实测收口（已拆分）

> **§6.1–§6.5 原文见 [docs/research/tectonic-lib-evidence.md](./research/tectonic-lib-evidence.md)**（编号不变；即 §6 P8 与 `.gitignore:124` 约定的「入库结论文档」）。本文 §7–§13 里出现的 `§6.x` 指针指该文件。
...
> 口径声明：本节只允许 `[实测]` 数字（指到 t5 的 evidence.jsonl 或既有实测）。**本方案不产生新的绝对毫秒数字**（跨 t5/t7 两批不可比，t7 任务契约）；每个数字必须连**夹具与命令口径**一起引（见 §8.2 注）。
...
| 项 | 拆分数字（t5 实测口径） | 答案在哪 |
|---|---|---|
| **排版段（CLI `--outfmt xdv`，3 次中位）** | **824 ms**（748/824/872） | §6.1 的 release 对照表（库形态 vs 子进程，另一份夹具） |
| **排版+转换（CLI `--outfmt pdf`，3 次中位）** | **1,155 ms**（1,072/1,155/1,224） | 同上 |
| ⇒ CLI 内转换增量 | **≈331 ms** | — |
| **进程内转换（`XdvipdfmxEngine`，release 3 次中位）** | **337 ms**（317/337/449）——与 CLI 增量差 ≈2%，两条腿互相印证 | — |
| 首编（冷缓存）/ 暖编（热 format） | **`[未测]`**（t5 未做冷/热对照） | §6.1：热 format 命中、冷缓存需生成 format 两条路都已实测 |
| 常驻复用（同进程连续编译 vs 每次起进程） | **已实测（本文 §6.4）：J1 不成立** —— 轻档（1 页）净收益 **−26 ms**、重档（28 页 Full）**+341 ms（+31%，第 3 轮起退化）**；进程地板本身只有 9.3 ms ⇒ 天花板远低于 P4 要求的 ≥100 ms。**按回退点处理：不投入常驻** | §6.4 |
| 内存峰值工作集 | 重夹具 **245 MB** / 轻夹具 **203 MB**（§6.4 的常驻档顺带量到） | §6.4；并发场景随 P4 结案一并搁置（LBR-7） |
| 二进制/磁盘体积 | 库形态增量 **`[未测到]`**（t4 §5.2） | 仍未测（§8.3） |
...
| F4 | 引擎崩溃（C abort） | 同进程可能整进程挂 | 结构性代价（`roadmap:244`；行号随 §6 拆分已漂移 ⇒ 现读作 [roadmap §6.5](./research/roadmap-item-specs.md)）：**必须**在 ADR 里写清「隔离能力下降」；UI 不承诺「编译失败=应用不崩」 |
...
| Z-5b | 页级复用 A/B/C 在库形态的恢复 | **已落地**（§6.3）：整份 XDV 路线下页哈希与子进程档同一个函数、逐页可比，A/B/C 在库形态档成立；"部分 PDF 复用"仍随 Z-5 搁置 |
...
| # | 风险 | 后果 | 现有处置 / 判据 | 依据 |
|---|---|---|---|---|
| LBR-1 | **构建链变重（C 依赖）** | CI 明显变重，Windows 需要 vcpkg；**21 个 native/C 链标记**（t4 §3.1），harfbuzz **随 crates.io 发布版内含源码 ⇒ 不踩空子模块坑**（t4 §3.3） | P1 阶段先做构建实测；不可行则停在子进程（N1） | `roadmap:244`（现读作 [roadmap §6.5](./research/roadmap-item-specs.md)）；t4 §3.1/§3.3（**t2 §7.1/§7.2 的"子模块硬 exit 1"只对 git 检出未 init 子模块成立，见 §3.1.3**） |
